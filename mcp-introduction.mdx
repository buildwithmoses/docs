...
description: "Learn how to install and configure the AirTrigger MCP Server"
---

## What is the AirTrigger MCP Server?

The AirTrigger MCP (Model Context Protocol) Server enables AI assistants to interact directly with your AirTrigger projects. It provides a comprehensive set of tools to:

- Search AirTrigger documentation
- Initialize new AirTrigger projects
...
The `search_docs` tool works without authentication. All other tools require you to be logged in via the [AirTrigger CLI](/cli-login-commands). The first time you use an authenticated tool, your MCP client will prompt you to log in.
...
- `-a, --api-url <value>` — Custom AirTrigger API URL
...
<CardGroup cols={2}>
  ...
  <Card title="Skills" icon="wand-magic-sparkles" href="/skills">
    Portable instruction sets that teach AI assistants AirTrigger patterns.
  </Card>
